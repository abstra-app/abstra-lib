...
class TestTables(unittest.TestCase):
    ...
    def test_crud_data(self):
        ...
        crud_table.insert(dict(
            name="Uzumaki Naruto",
            email="uzumaki.naruto@konoha.jp"
        ))
        crud_table.insert(dict(
            name="Uchiha Sasuke",
            email="uchiha.sasuke@konoha.jp"
        ))
        crud_table.insert(dict(
            name="Haruno Sakura",
            email="haruno.sakura@konoha.jp"
        ))
        crud_table.insert(dict(
            name="Hatake Kakashi",
            email="hatake.kakashi@konoha.jp"
        ))

        self.assertEqual(len(crud_table.select()), 8)
        self.assertEqual(len(crud_table.select(where="email LIKE :email_like", params=dict(email_like='%@dundermifflin.com'))), 4)
        self.assertEqual(crud_table.select(columns=["name"], where="email = :email", params=dict(email="uzumaki.naruto@konoha.jp")),[
            dict(name="Uzumaki Naruto")
        ])

        crud_table.update(where="email = :email", params=dict(email="uchiha.sasuke@konoha.jp"), set=dict(email="uchiha.sasuke@akatsuki.jp"))

        self.assertEqual(crud_table.select(columns=["email"], where="name = :name", params=dict(name="Uchiha Sasuke")),[
            dict(email="uchiha.sasuke@akatsuki.jp")
        ])

        crud_table.delete(where="email = :email", params=dict(email="uchiha.sasuke@akatsuki.jp"))
