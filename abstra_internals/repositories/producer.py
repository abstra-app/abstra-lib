...
class RabbitMQProducerRepository(ProducerRepository):
        ...
        for attempt in range(1, RABBITMQ_RETRY_MAX_ATTEMPTS + 1):
            try:
                ...
                if attempt < RABBITMQ_RETRY_MAX_ATTEMPTS:
                    ...
                    AbstraLogger.error(
                        f"[RabbitMQProducerRepository] All {RABBITMQ_RETRY_MAX_ATTEMPTS} connection attempts failed"
                    )
    ...
    def enqueue(self, stage_id: str, context: ClientContext) -> Connection:
        ...
        # Create RabbitMQ-based connection for bidirectional communication
        # Server sends to "server_to_worker" and receives from "worker_to_server"
        ...
        return rabbitmq_connection  # type: ignore
