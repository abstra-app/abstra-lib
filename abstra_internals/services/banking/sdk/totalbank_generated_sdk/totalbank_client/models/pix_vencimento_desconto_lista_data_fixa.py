...
class PixVencimentoDescontoListaDataFixa(BaseModel):
    """
    Lista de descontos
    """  # noqa: E501

    desconto_data_fixa: Optional[List[PixVencimentoDescontoDataFixa]] = Field(
        default=None, alias="descontoDataFixa"
    )
    __properties: ClassVar[List[str]] = ["descontoDataFixa"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PixVencimentoDescontoListaDataFixa from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in desconto_data_fixa (list)
        _items = []
        if self.desconto_data_fixa:
            for _item_desconto_data_fixa in self.desconto_data_fixa:
                if _item_desconto_data_fixa:
                    _items.append(_item_desconto_data_fixa.to_dict())
            _dict["descontoDataFixa"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PixVencimentoDescontoListaDataFixa from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "descontoDataFixa": [
                    PixVencimentoDescontoDataFixa.from_dict(_item)
                    for _item in obj["descontoDataFixa"]
                ]
                if obj.get("descontoDataFixa") is not None
                else None
            }
        )
        return _obj
