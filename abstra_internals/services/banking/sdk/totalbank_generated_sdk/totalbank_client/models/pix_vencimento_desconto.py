...
PIXVENCIMENTODESCONTO_ONE_OF_SCHEMAS = [
    "PixVencimentoDescontoListaDataFixa",
    "PixVencimentoDescontoValorPerc",
]


class PixVencimentoDesconto(BaseModel):
    """
    Descontos aplicados à cobrança
    """

    # data type: PixVencimentoDescontoValorPerc
    oneof_schema_1_validator: Optional[PixVencimentoDescontoValorPerc] = None
    # data type: PixVencimentoDescontoListaDataFixa
    oneof_schema_2_validator: Optional[PixVencimentoDescontoListaDataFixa] = None
    actual_instance: Optional[
        Union[PixVencimentoDescontoListaDataFixa, PixVencimentoDescontoValorPerc]
    ] = None
    ...
    def actual_instance_must_validate_oneof(cls, v):
        PixVencimentoDesconto.model_construct()
        error_messages = []
        match = 0
        # validate data type: PixVencimentoDescontoValorPerc
        if not isinstance(v, PixVencimentoDescontoValorPerc):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `PixVencimentoDescontoValorPerc`"
            )
        else:
            match += 1
        # validate data type: PixVencimentoDescontoListaDataFixa
        if not isinstance(v, PixVencimentoDescontoListaDataFixa):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `PixVencimentoDescontoListaDataFixa`"
            )
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in PixVencimentoDesconto with oneOf schemas: PixVencimentoDescontoListaDataFixa, PixVencimentoDescontoValorPerc. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in PixVencimentoDesconto with oneOf schemas: PixVencimentoDescontoListaDataFixa, PixVencimentoDescontoValorPerc. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into PixVencimentoDescontoValorPerc
        try:
            instance.actual_instance = PixVencimentoDescontoValorPerc.from_json(
                json_str
            )
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into PixVencimentoDescontoListaDataFixa
        try:
            instance.actual_instance = PixVencimentoDescontoListaDataFixa.from_json(
                json_str
            )
            match += 1
        except (ValidationError, ValueError) as e:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into PixVencimentoDesconto with oneOf schemas: PixVencimentoDescontoListaDataFixa, PixVencimentoDescontoValorPerc. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into PixVencimentoDesconto with oneOf schemas: PixVencimentoDescontoListaDataFixa, PixVencimentoDescontoValorPerc. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
