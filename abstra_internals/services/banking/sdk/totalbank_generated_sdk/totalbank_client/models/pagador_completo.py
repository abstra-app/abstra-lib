...
class PagadorCompleto(BaseModel):
    """
    Pagadores
    """  # noqa: E501
    ...
    enderecos: Optional[List[Endereco]] = Field(
        default=None, description="Endereços do pagador"
    )
    avalistas: Optional[List[AvalistaPagador]] = Field(
        default=None, description="Avalistas do pagador"
    )
    contatos: Optional[List[ContatoPagador]] = Field(
        default=None, description="Contatos do pagador"
    )
    telefone: Optional[StrictStr] = Field(
        default=None, description="Telefone do pagador"
    )
    email: Optional[StrictStr] = Field(default=None, description="E-mail do pagador")
    apelido: Optional[StrictStr] = Field(default=None, description="Apelido")
    nome_mae: Optional[StrictStr] = Field(
        default=None, description="Nome da mãe", alias="nomeMae"
    )
    nome_pai: Optional[StrictStr] = Field(
        default=None, description="Nome do pai", alias="nomePai"
    )
    data_nascimento: Optional[date] = Field(
        default=None,
        description="Data de nascimento ou fundação (formato aaaa-mm-dd)",
        alias="dataNascimento",
    )
    observacoes: Optional[StrictStr] = Field(default=None, description="Observações")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PagadorCompleto from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of pessoa
        if self.pessoa:
            _dict["pessoa"] = self.pessoa.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in enderecos (list)
        _items = []
        if self.enderecos:
            for _item_enderecos in self.enderecos:
                if _item_enderecos:
                    _items.append(_item_enderecos.to_dict())
            _dict["enderecos"] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in avalistas (list)
        _items = []
        if self.avalistas:
            for _item_avalistas in self.avalistas:
                if _item_avalistas:
                    _items.append(_item_avalistas.to_dict())
            _dict["avalistas"] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in contatos (list)
        _items = []
        if self.contatos:
            for _item_contatos in self.contatos:
                if _item_contatos:
                    _items.append(_item_contatos.to_dict())
            _dict["contatos"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PagadorCompleto from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "pessoa": Pessoa.from_dict(obj["pessoa"])
                if obj.get("pessoa") is not None
                else None,
                "enderecos": [Endereco.from_dict(_item) for _item in obj["enderecos"]]
                if obj.get("enderecos") is not None
                else None,
                "avalistas": [
                    AvalistaPagador.from_dict(_item) for _item in obj["avalistas"]
                ]
                if obj.get("avalistas") is not None
                else None,
                "contatos": [
                    ContatoPagador.from_dict(_item) for _item in obj["contatos"]
                ]
                if obj.get("contatos") is not None
                else None,
                "telefone": obj.get("telefone"),
                "email": obj.get("email"),
                "apelido": obj.get("apelido"),
                "nomeMae": obj.get("nomeMae"),
                "nomePai": obj.get("nomePai"),
                "dataNascimento": obj.get("dataNascimento"),
