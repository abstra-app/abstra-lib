...
class RetornoListaPagadorVO(BaseModel):
    """
    RetornoListaPagadorVO
    """  # noqa: E501

    pagadores: Optional[List[PagadorCompleto]] = Field(
        default=None, description="Pagadores"
    )
    __properties: ClassVar[List[str]] = ["pagadores"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RetornoListaPagadorVO from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in pagadores (list)
        _items = []
        if self.pagadores:
            for _item_pagadores in self.pagadores:
                if _item_pagadores:
                    _items.append(_item_pagadores.to_dict())
            _dict["pagadores"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RetornoListaPagadorVO from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "pagadores": [
                    PagadorCompleto.from_dict(_item) for _item in obj["pagadores"]
                ]
                if obj.get("pagadores") is not None
                else None
            }
        )
        return _obj
