...
class PixConcluido(BaseModel):
    """
    Os campos aninhados sob o objeto devedor são opcionais e identificam o devedor, ou seja, a pessoa ou a instituição a quem a cobrança está endereçada. Não identifica, necessariamente, quem irá efetivamente realizar o pagamento. Um CPF pode ser o devedor de uma cobrança, mas pode acontecer de outro CPF realizar, efetivamente, o pagamento do documento. Se o campo devedor.nome está preenchido, então deve existir um campo devedor.inscricao preenchido.
    """  # noqa: E501
    ...
    valor: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="Valor do pix"
    )
    horario: Optional[datetime] = Field(
        default=None,
        description="Horário de liquidação. (formato yyyy-MM-dd HH:mm:ss.SSS)",
    )
    inscricao_pagador_efetivo: Optional[StrictStr] = Field(
        default=None,
        description="Inscrição do pagador efetivo.",
        alias="inscricaoPagadorEfetivo",
    )
    nome_pagador_efetivo: Optional[StrictStr] = Field(
        default=None, description="Nome do pagador efetivo.", alias="nomePagadorEfetivo"
    )
    info_pagador: Optional[StrictStr] = Field(
        default=None, description="Informação livre do pagador.", alias="infoPagador"
    )
    ...
    devolucoes: Optional[List[RetornoPixDevolucaoVO]] = Field(
        default=None, description="Devoluções das transações PIX"
    )
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PixConcluido from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in devolucoes (list)
        _items = []
        if self.devolucoes:
            for _item_devolucoes in self.devolucoes:
                if _item_devolucoes:
                    _items.append(_item_devolucoes.to_dict())
            _dict["devolucoes"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PixConcluido from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                ...
                "valor": obj.get("valor"),
                "horario": obj.get("horario"),
                "inscricaoPagadorEfetivo": obj.get("inscricaoPagadorEfetivo"),
                "nomePagadorEfetivo": obj.get("nomePagadorEfetivo"),
                "infoPagador": obj.get("infoPagador"),
                "meioInformacaoLiquidacao": obj.get("meioInformacaoLiquidacao"),
                "devolucoes": [
                    RetornoPixDevolucaoVO.from_dict(_item)
                    for _item in obj["devolucoes"]
                ]
                if obj.get("devolucoes") is not None
                else None,
            }
        )
        return _obj
