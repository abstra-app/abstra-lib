...
class ParametroPagamentoVO(BaseModel):
    """
    Dados do parâmetro de pagamento, específico para pagador Caixa (104)
    """  # noqa: E501

    codigo_compromisso: Optional[Annotated[int, Field(le=9999, strict=True, ge=1)]] = (
        Field(
            default=None,
            description="Código do compromisso. O número do compromisso possibilita customizar parâmetros do serviço sem a necessidade de utilização de novo convênio. Por exemplo: o   compromisso “1” pode utilizar históricos de lançamentos diferentes do compromisso “2”.  Essas customizações podem auxiliar a conciliação contábil da empresa.",
            alias="codigoCompromisso",
        )
    )
    tipo_compromisso: Optional[StrictStr] = Field(
        default=None,
        description="Tipo do compromisso. O tipo do compromisso caracteriza qual a finalidade dos pagamentos/transações. Tipos disponíveis no enum",
        alias="tipoCompromisso",
    )
    parametro_transmissao: Optional[Annotated[int, Field(le=99, strict=True, ge=1)]] = (
        Field(
            default=None,
            description="Parâmetro de transmissão",
            alias="parametroTransmissao",
        )
    )
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ParametroPagamentoVO from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ParametroPagamentoVO from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "codigoCompromisso": obj.get("codigoCompromisso"),
                "tipoCompromisso": obj.get("tipoCompromisso"),
                "parametroTransmissao": obj.get("parametroTransmissao"),
            }
        )
        return _obj
