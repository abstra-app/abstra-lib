...
class RetornoPesquisaOcorrenciaPaginada(BaseModel):
    """
    RetornoPesquisaOcorrenciaPaginada
    """  # noqa: E501

    codigo_retorno: Optional[StrictInt] = Field(
        default=None, description="Código de retorno", alias="codigoRetorno"
    )
    mensagem_retorno: Optional[StrictStr] = Field(
        default=None, description="Mensagem", alias="mensagemRetorno"
    )
    ocorrencias: Optional[List[OcorrenciaHistorico]] = Field(
        default=None, description="Ocorrências dos títulos"
    )
    paginacao: Optional[RetornoPaginacao] = None
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RetornoPesquisaOcorrenciaPaginada from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in ocorrencias (list)
        _items = []
        if self.ocorrencias:
            for _item_ocorrencias in self.ocorrencias:
                if _item_ocorrencias:
                    _items.append(_item_ocorrencias.to_dict())
            _dict["ocorrencias"] = _items
        # override the default output from pydantic by calling `to_dict()` of paginacao
        if self.paginacao:
            _dict["paginacao"] = self.paginacao.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RetornoPesquisaOcorrenciaPaginada from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "codigoRetorno": obj.get("codigoRetorno"),
                "mensagemRetorno": obj.get("mensagemRetorno"),
                "ocorrencias": [
                    OcorrenciaHistorico.from_dict(_item) for _item in obj["ocorrencias"]
                ]
                if obj.get("ocorrencias") is not None
                else None,
                "paginacao": RetornoPaginacao.from_dict(obj["paginacao"])
                if obj.get("paginacao") is not None
                else None,
            }
        )
        return _obj
