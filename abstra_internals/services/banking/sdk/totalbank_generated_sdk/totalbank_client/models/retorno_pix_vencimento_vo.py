...
class RetornoPixVencimentoVO(BaseModel):
    """
    Cobrança com vencimento
    """  # noqa: E501

    chave: Optional[StrictStr] = Field(
        default=None, description="Chave DICT do recebedor"
    )
    solicitacao_pagador: Optional[StrictStr] = Field(
        default=None, description="Solicitação ao pagador", alias="solicitacaoPagador"
    )
    info_adicionais: Optional[List[PixInfoAdicional]] = Field(
        default=None, description="Informações adicionais", alias="infoAdicionais"
    )
    devedor: Optional[PixVencimentoDevedor] = None
    valor: Optional[PixVencimentoValor] = None
    calendario: Optional[RetornoPixVencimentoCalendario] = None
    location: Optional[StrictStr] = Field(default=None, description="Link do pix")
    pix_copia_e_cola: Optional[StrictStr] = Field(
        default=None, description="Imagem qrcode", alias="pixCopiaECola"
    )
    revisao: Optional[StrictInt] = Field(default=None, description="Revisão")
    txid: Optional[StrictStr] = Field(
        default=None, description="Identificador da transação"
    )
    status: Optional[StrictStr] = Field(default=None, description="Status da cobrança")
    pix_concluido: Optional[PixConcluido] = Field(default=None, alias="pixConcluido")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RetornoPixVencimentoVO from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in info_adicionais (list)
        _items = []
        if self.info_adicionais:
            for _item_info_adicionais in self.info_adicionais:
                if _item_info_adicionais:
                    _items.append(_item_info_adicionais.to_dict())
            _dict["infoAdicionais"] = _items
        # override the default output from pydantic by calling `to_dict()` of devedor
        if self.devedor:
            _dict["devedor"] = self.devedor.to_dict()
        # override the default output from pydantic by calling `to_dict()` of valor
        if self.valor:
            _dict["valor"] = self.valor.to_dict()
        # override the default output from pydantic by calling `to_dict()` of calendario
        if self.calendario:
            _dict["calendario"] = self.calendario.to_dict()
        # override the default output from pydantic by calling `to_dict()` of pix_concluido
        if self.pix_concluido:
            _dict["pixConcluido"] = self.pix_concluido.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RetornoPixVencimentoVO from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                ...
                "infoAdicionais": [
                    PixInfoAdicional.from_dict(_item) for _item in obj["infoAdicionais"]
                ]
                if obj.get("infoAdicionais") is not None
                else None,
                "devedor": PixVencimentoDevedor.from_dict(obj["devedor"])
                if obj.get("devedor") is not None
                else None,
                "valor": PixVencimentoValor.from_dict(obj["valor"])
                if obj.get("valor") is not None
                else None,
                "calendario": RetornoPixVencimentoCalendario.from_dict(
                    obj["calendario"]
                )
                if obj.get("calendario") is not None
                else None,
                "location": obj.get("location"),
                "pixCopiaECola": obj.get("pixCopiaECola"),
                "revisao": obj.get("revisao"),
                "txid": obj.get("txid"),
                "status": obj.get("status"),
                "pixConcluido": PixConcluido.from_dict(obj["pixConcluido"])
                if obj.get("pixConcluido") is not None
                else None,
            }
        )
        return _obj
