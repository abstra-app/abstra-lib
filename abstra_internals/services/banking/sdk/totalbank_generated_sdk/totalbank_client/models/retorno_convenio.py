...
class RetornoConvenio(BaseModel):
    ...
    convenios: Optional[List[ConvenioCompletoVO]] = None
    __properties: ClassVar[List[str]] = ["convenios"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RetornoConvenio from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in convenios (list)
        _items = []
        if self.convenios:
            for _item_convenios in self.convenios:
                if _item_convenios:
                    _items.append(_item_convenios.to_dict())
            _dict["convenios"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RetornoConvenio from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "convenios": [
                    ConvenioCompletoVO.from_dict(_item) for _item in obj["convenios"]
                ]
                if obj.get("convenios") is not None
                else None
            }
        )
        return _obj
