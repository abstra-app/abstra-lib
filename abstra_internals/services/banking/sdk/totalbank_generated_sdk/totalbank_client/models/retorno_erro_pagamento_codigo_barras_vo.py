...
class RetornoErroPagamentoCodigoBarrasVO(BaseModel):
    """
    RetornoErroPagamentoCodigoBarrasVO
    """  # noqa: E501

    erros: Optional[List[ListaErroCodigoBarrasVO]] = Field(
        default=None, description="Lista de erros."
    )
    __properties: ClassVar[List[str]] = ["erros"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RetornoErroPagamentoCodigoBarrasVO from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in erros (list)
        _items = []
        if self.erros:
            for _item_erros in self.erros:
                if _item_erros:
                    _items.append(_item_erros.to_dict())
            _dict["erros"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RetornoErroPagamentoCodigoBarrasVO from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "erros": [
                    ListaErroCodigoBarrasVO.from_dict(_item) for _item in obj["erros"]
                ]
                if obj.get("erros") is not None
                else None
            }
        )
        return _obj
