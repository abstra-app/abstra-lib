...
class ChavePix(BaseModel):
    """
    As chaves dependem do tipo da chave escolhido. Caso CPFCNPJ, o campo cpfCnpj precisa ser preenchido. Caso TELEFONE, o campo telefone. Caso CHAVE_ALEATORIA, campo chaveAleatoria. Caso DADOS_BANCARIOS, os campos banco, agencia, contaCorrente e dvContaCorrente precisam ser preenchidos.
    """

    # data type: ChaveCpfCnpj
    oneof_schema_1_validator: Optional[ChaveCpfCnpj] = None
    # data type: ChaveEmail
    oneof_schema_2_validator: Optional[ChaveEmail] = None
    # data type: ChaveTelefone
    oneof_schema_3_validator: Optional[ChaveTelefone] = None
    # data type: ChaveAleatoria
    oneof_schema_4_validator: Optional[ChaveAleatoria] = None
    # data type: ChaveDadosBancarios
    oneof_schema_5_validator: Optional[ChaveDadosBancarios] = None
    actual_instance: Optional[
        Union[
            ChaveAleatoria, ChaveCpfCnpj, ChaveDadosBancarios, ChaveEmail, ChaveTelefone
        ]
    ] = None
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ChavePix.model_construct()
        error_messages = []
        match = 0
        # validate data type: ChaveCpfCnpj
        if not isinstance(v, ChaveCpfCnpj):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ChaveCpfCnpj`"
            )
        else:
            match += 1
        # validate data type: ChaveEmail
        if not isinstance(v, ChaveEmail):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ChaveEmail`")
        else:
            match += 1
        # validate data type: ChaveTelefone
        if not isinstance(v, ChaveTelefone):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ChaveTelefone`"
            )
        else:
            match += 1
        # validate data type: ChaveAleatoria
        if not isinstance(v, ChaveAleatoria):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ChaveAleatoria`"
            )
        else:
            match += 1
        # validate data type: ChaveDadosBancarios
        if not isinstance(v, ChaveDadosBancarios):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ChaveDadosBancarios`"
            )
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in ChavePix with oneOf schemas: ChaveAleatoria, ChaveCpfCnpj, ChaveDadosBancarios, ChaveEmail, ChaveTelefone. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in ChavePix with oneOf schemas: ChaveAleatoria, ChaveCpfCnpj, ChaveDadosBancarios, ChaveEmail, ChaveTelefone. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ChaveCpfCnpj
        try:
            instance.actual_instance = ChaveCpfCnpj.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ChaveEmail
        try:
            instance.actual_instance = ChaveEmail.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ChaveTelefone
        try:
            instance.actual_instance = ChaveTelefone.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ChaveAleatoria
        try:
            instance.actual_instance = ChaveAleatoria.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ChaveDadosBancarios
        try:
            instance.actual_instance = ChaveDadosBancarios.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into ChavePix with oneOf schemas: ChaveAleatoria, ChaveCpfCnpj, ChaveDadosBancarios, ChaveEmail, ChaveTelefone. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ChavePix with oneOf schemas: ChaveAleatoria, ChaveCpfCnpj, ChaveDadosBancarios, ChaveEmail, ChaveTelefone. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
