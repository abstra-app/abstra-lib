...
class FavorecidoTransferenciaVO(BaseModel):
    """
    Dados do favorecido
    """  # noqa: E501

    cpf_cnpj: Optional[StrictStr] = Field(
        default=None, description="CPF/CNPJ", alias="cpfCnpj"
    )
    nome_razao_social: Optional[StrictStr] = Field(
        default=None,
        description="Nome ou razão social do favorecido",
        alias="nomeRazaoSocial",
    )
    endereco_favorecido: Optional[EnderecoVO] = Field(
        default=None, alias="enderecoFavorecido"
    )
    conta_credito: Optional[ContaCreditoVO] = Field(default=None, alias="contaCredito")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of FavorecidoTransferenciaVO from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of endereco_favorecido
        if self.endereco_favorecido:
            _dict["enderecoFavorecido"] = self.endereco_favorecido.to_dict()
        # override the default output from pydantic by calling `to_dict()` of conta_credito
        if self.conta_credito:
            _dict["contaCredito"] = self.conta_credito.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of FavorecidoTransferenciaVO from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                ...
                "enderecoFavorecido": EnderecoVO.from_dict(obj["enderecoFavorecido"])
                if obj.get("enderecoFavorecido") is not None
                else None,
                "contaCredito": ContaCreditoVO.from_dict(obj["contaCredito"])
                if obj.get("contaCredito") is not None
                else None,
            }
        )
        return _obj
