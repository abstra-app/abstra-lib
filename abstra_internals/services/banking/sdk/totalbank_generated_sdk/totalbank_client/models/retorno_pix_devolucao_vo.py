...
class RetornoPixDevolucaoVO(BaseModel):
    """
    Dados da devolução
    """  # noqa: E501

    id: Optional[StrictStr] = Field(
        default=None, description="Id que representa unicamente uma devolução."
    )
    rtr_id: Optional[StrictStr] = Field(
        default=None,
        description="ReturnIdentification que transita na PACS004.",
        alias="rtrId",
    )
    valor: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="Valor a devolver."
    )
    status: Optional[StrictStr] = Field(default=None, description="Status da devolução")
    motivo: Optional[StrictStr] = Field(
        default=None,
        description="Campo que pode ser utilizado pelo PSP recebedor para detalhar os motivos de a devolução ter atingido o status em questão.",
    )
    solicitacao_devolucao: Optional[datetime] = Field(
        default=None,
        description="Horário no qual a devolução foi solicitada",
        alias="solicitacaoDevolucao",
    )
    liquidacao_devolucao: Optional[datetime] = Field(
        default=None,
        description="Horário no qual a devolução foi liquidada no PSP",
        alias="liquidacaoDevolucao",
    )
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RetornoPixDevolucaoVO from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RetornoPixDevolucaoVO from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                ...
                "solicitacaoDevolucao": obj.get("solicitacaoDevolucao"),
                "liquidacaoDevolucao": obj.get("liquidacaoDevolucao"),
            }
        )
        return _obj
