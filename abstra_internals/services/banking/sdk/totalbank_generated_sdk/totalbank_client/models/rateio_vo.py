...
class RateioVO(BaseModel):
    """
    Rateio
    """  # noqa: E501

    tipo_valor: Optional[StrictStr] = Field(
        default=None, description="Tipo de valor", alias="tipoValor"
    )
    tipo_calculo: Optional[StrictStr] = Field(
        default=None, description="Tipo cálculo do rateio", alias="tipoCalculo"
    )
    beneficiarios: Optional[List[RateioBeneficiarioVO]] = Field(
        default=None, description="Beneficiários do rateio"
    )
    __properties: ClassVar[List[str]] = ["tipoValor", "tipoCalculo", "beneficiarios"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RateioVO from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in beneficiarios (list)
        _items = []
        if self.beneficiarios:
            for _item_beneficiarios in self.beneficiarios:
                if _item_beneficiarios:
                    _items.append(_item_beneficiarios.to_dict())
            _dict["beneficiarios"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RateioVO from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "tipoValor": obj.get("tipoValor"),
                "tipoCalculo": obj.get("tipoCalculo"),
                "beneficiarios": [
                    RateioBeneficiarioVO.from_dict(_item)
                    for _item in obj["beneficiarios"]
                ]
                if obj.get("beneficiarios") is not None
                else None,
            }
        )
        return _obj
