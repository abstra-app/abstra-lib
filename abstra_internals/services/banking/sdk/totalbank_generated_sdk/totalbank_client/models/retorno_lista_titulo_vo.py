...
class RetornoListaTituloVO(BaseModel):
    """
    RetornoListaTituloVO
    """  # noqa: E501

    retornos: Optional[List[RetornoPesquisa]] = Field(
        default=None, description="Retornos"
    )
    __properties: ClassVar[List[str]] = ["retornos"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RetornoListaTituloVO from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in retornos (list)
        _items = []
        if self.retornos:
            for _item_retornos in self.retornos:
                if _item_retornos:
                    _items.append(_item_retornos.to_dict())
            _dict["retornos"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RetornoListaTituloVO from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "retornos": [
                    RetornoPesquisa.from_dict(_item) for _item in obj["retornos"]
                ]
                if obj.get("retornos") is not None
                else None
            }
        )
        return _obj
