...
class RequisicaoMovimentaTituloValorNominalVO(BaseModel):
    """
    RequisicaoMovimentaTituloValorNominalVO
    """  # noqa: E501

    beneficiario: Optional[Beneficiario] = None
    titulos: Optional[List[FiltroMovimentacaoTitulo]] = Field(
        default=None, description="Títulos"
    )
    valor_nominal: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="Valor nominal do título", alias="valorNominal"
    )
    __properties: ClassVar[List[str]] = ["beneficiario", "titulos", "valorNominal"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RequisicaoMovimentaTituloValorNominalVO from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of beneficiario
        if self.beneficiario:
            _dict["beneficiario"] = self.beneficiario.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in titulos (list)
        _items = []
        if self.titulos:
            for _item_titulos in self.titulos:
                if _item_titulos:
                    _items.append(_item_titulos.to_dict())
            _dict["titulos"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RequisicaoMovimentaTituloValorNominalVO from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "beneficiario": Beneficiario.from_dict(obj["beneficiario"])
                if obj.get("beneficiario") is not None
                else None,
                "titulos": [
                    FiltroMovimentacaoTitulo.from_dict(_item)
                    for _item in obj["titulos"]
                ]
                if obj.get("titulos") is not None
                else None,
                "valorNominal": obj.get("valorNominal"),
            }
        )
        return _obj
