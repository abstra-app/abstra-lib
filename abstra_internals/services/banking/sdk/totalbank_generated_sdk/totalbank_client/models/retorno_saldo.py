...
class RetornoSaldo(BaseModel):
    """
    RetornoSaldo
    """  # noqa: E501

    saldos: Optional[List[SaldoVO]] = Field(default=None, description="Saldo")
    __properties: ClassVar[List[str]] = ["saldos"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RetornoSaldo from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in saldos (list)
        _items = []
        if self.saldos:
            for _item_saldos in self.saldos:
                if _item_saldos:
                    _items.append(_item_saldos.to_dict())
            _dict["saldos"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RetornoSaldo from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "saldos": [SaldoVO.from_dict(_item) for _item in obj["saldos"]]
                if obj.get("saldos") is not None
                else None
            }
        )
        return _obj
